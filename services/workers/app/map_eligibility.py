"""Mirror of API map eligibility rules (workers must not import services/api)."""
...
def _has_display_or_kpi_signals(payload: dict[str, Any], kpi_json: dict[str, Any] | None) -> bool:
    ...
    if isinstance(kpi.get("displayFields"), (dict, list)):
        if isinstance(kpi.get("displayFields"), dict) and kpi["displayFields"]:
            return True
        if isinstance(kpi.get("displayFields"), list) and kpi["displayFields"]:
            return True
    met = kpi.get("metrics")
...
def map_eligible_result_object(
    *,
    ...
    hs = payload.get("health_status") if isinstance(payload.get("health_status"), str) else None
    content = (
        _has_display_or_kpi_signals(payload, empty_kpi)
        or _has_health_signal(payload, hs, False)
