...
def get_enterprise_dashboard(
    ...
    """Resolved dashboard: valid primary frozen layout, else synthetic Operations Overview."""
    rds = get_redis()
    pref = db.get(DashboardUserPreference, user.id)
    pid = pref.primary_dashboard_id if pref else None
    if pid and rds:
        cache_key = _ent_dash_cache_key(user.id, pid)
        try:
            raw = rds.get(cache_key)
            if raw:
                cached = DashboardLiveResponse.model_validate_json(raw)
                if not cached.is_default_dashboard and cached.primary_dashboard_id == pid:
                    return cached
        except Exception:
            log.debug("enterprise dashboard cache read failed", exc_info=True)

    resp = build_dashboard_live_response(db, user)

    if resp.is_default_dashboard and pid and rds:
        try:
            rds.delete(_ent_dash_cache_key(user.id, pid))
        except Exception:
            log.debug("enterprise dashboard cache delete failed", exc_info=True)

    if resp.is_default_dashboard:
        pipeline_emit(
            log,
            component="api.enterprise_dashboard",
            action="default_dashboard",
            status="ok",
            user_id=str(user.id),
        )
        return resp

    if resp.primary_dashboard_id and rds:
        cache_key = _ent_dash_cache_key(user.id, resp.primary_dashboard_id)
        try:
    ...
    pipeline_emit(
        log,
        component="api.enterprise_dashboard",
        action="resolved_live",
        status="ok",
        dashboard_id=str(resp.primary_dashboard_id) if resp.primary_dashboard_id else None,
        is_default=str(resp.is_default_dashboard),
    )
    return resp
